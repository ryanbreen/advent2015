...
fn part2 (input: String) -> String {
  ...
  for line in lines {
    ...
    for connection_part in &connection_parts {
      ...
      connections.insert(Connection {net_weight: 0, partner: "me".to_string()});
      connections.insert(Connection { net_weight: connection_weight, partner: connection_part.1.to_string() });
    }
  }

  let mut me = Person { connections: Box::new(BTreeSet::new()) };

  {
    let ref mut my_connections:BTreeSet<Connection> = *(me.connections.borrow_mut());
    for (name, _) in &people {
      my_connections.insert(Connection {net_weight: 0, partner: name.to_string()});
    }
  }

  people.insert("me".to_string(), me);
